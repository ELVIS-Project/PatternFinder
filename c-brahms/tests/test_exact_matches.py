...
class CBRAHMSTestExactMatches(TestCase):
    ...
    def test_P1_edgecase_pattern_larger_than_source(self):
        list_of_shifts = cbrahmsGeo.P1(self.pattern, self.source[0:4], 'onset')
        self.assertEqual(list_of_shifts, [])
    def test_P2_edgecase_pattern_larger_than_source(self):
        list_of_shifts = self.P2(self.pattern, self.source[0:4], 0)
        self.assertEqual(list_of_shifts, [])
    ...
    def test_P1_edgecase_identical_source(self):
        list_of_shifts = self.P1(self.pattern, self.source, 'onset')
        self.assertEqual(list_of_shifts, [TwoDVector(0,0)])
    def test_P2_edgecase_identical_source(self):
        list_of_shifts = self.P2(self.pattern, self.source, 0)
        self.assertEqual(list_of_shifts, [TwoDVector(0,0)])
