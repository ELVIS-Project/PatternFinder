...
class CBRAHMSTestP2(TestCase):
    ...
    def test_P2_edgecase_same_size_transposed(self):
        """
        Checks if P2 can match a pattern to a source of the same size, but transposed.
        """
        # transpose pattern 12 semitones upwards
        shift = [0,12]
        source = tools.shift_pattern(shift, self.pattern)
        list_of_shifts = cbrahmsGeo.P2(self.pattern, source)
        self.assertEqual(list_of_shifts, [shift])

    def test_P2_edgecase_identical_source(self):
        """
        Checks if P2 can match a pattern with an identical source
        """
    ...
    def test_P2_repeated_pattern_in_source(self):
        """
        Translates a pattern automatically, and checks to see if P2 can find all of the translations
        """
        source = list(self.pattern)
    ...
    def test_P2_pattern_larger_than_source(self):
        """
        Tests P2 with a pattern that is larger than the source. It should return an empty list.
        """
    ...
    def test_P2_midiparser_lemstrom(self):
        """
        Parses the ground truth polyphonic music example provided in Lemstrom and Laitninen's 2011 paper. There should be only one exact occurrence found by P2.
        """
        list_of_shifts = tools.run_algorithm_with_midiparser(cbrahmsGeo.P2, self.lemstrom_query('a'), self.lemstrom_source)
    ...
    def test_P2_midiparser_chidori(self):
        ...
        """
        list_of_shifts = tools.run_algorithm_with_midiparser(cbrahmsGeo.P2, 'music_files/chidori_query.mid', 'music_files/chidori_meimei.mid')
        self.assertEqual(list_of_shifts, [[2.0, -10], [6.0, -10], [65.0, -10], [69.0, -10]])


    def test_P2_midiparser_bwv2(self):
        list_of_shifts = tools.run_algorithm_with_midiparser(cbrahmsGeo.P2, 'music_files/query_V-i.mid', 'music_files/bach_BWV2_chorale.krn')
